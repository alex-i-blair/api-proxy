...
exports.handler = async (event) => {
  try {
    ...
    const response = await fetch(
      `http://api.openweathermap.org/geo/1.0/direct?q=${city},${state},${country}&limit=1&appid=${process.env.WEATHER_KEY}`
    );
    const geoCodingJson = await response.json();
    const latitude = geoCodingJson[0].lat;
    const longitude = geoCodingJson[0].lon;
    const weather = await fetch(
      `http://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&units=imperial&appid=${process.env.WEATHER_KEY}`
    );
    ...
    return {
      ...
      body: JSON.stringify(weatherJson.main),
    };
  } catch (error) {
    // console.log(error);
    return {
